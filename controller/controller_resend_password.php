  ...
  $fileDirs = dirname(__FILE__)."/../modular/autoload_apps.php";
   ...
   if($user1 != NULL )
    ...
    if($userExist != NULL )
    {
        ...
        $aboutJOP = "Welcome to Jury of peers Member , you send us request to change your password ! please click in the following link to do it  " ;
        $activationCode = $activationCoded->activation_code ;
        $linkActivationCode= "http://juryofpeers.tv/update_password?activation_code=".$activationCode."&username=". $user_name ; 
        $fromEmail= "Juryofpeers@juryofpeers.tv" ;
        $toEmail= $userExist->e_mail ;
        $subjects = "Rechange password request" ;
        if($activationCoded != NULL ){
            $is_sent =   $general_api->send_changePassword_to_usermail($logoSrc, $user_name, $aboutJOP,$activationCode , $linkActivationCode, $fromEmail, $toEmail, $subjects);
            if($is_sent)
                echo "1";
        }
